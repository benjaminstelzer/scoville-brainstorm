...
Status: v2 frozen-design draft for W-002, 2026-08-11

This version supersedes the unexecuted qualification use of v1 after one open single-session Train diagnostic showed that v1 exact Gold rejected semantically valid open forms such as `English` versus `en`, descriptive constraint arrays versus bare IDs, and descriptive selection requirements versus booleans. The raw v1 run remains evidence and is classified as a benchmark-contract diagnostic. No ADHD, candidate, Validation, or Test run used v1.
